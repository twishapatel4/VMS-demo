...
class BYTETracker:
    ...
    def update(self, output_results):
        """output_results: np.ndarray [N, 5] = (x1, y1, x2, y2, score)."""
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        if len(dets) > 0:
            detections = [STrack(STrack.tlbr_to_tlwh(t), s) for t, s in zip(dets, scores_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            detections_second = [STrack(STrack.tlbr_to_tlwh(t), s) for t, s in zip(dets_second, scores_second)]
        else:
            detections_second = []
        ...
        # Deal with unconfirmed tracks (new tracks alive only 1 frame)
